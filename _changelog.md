## [2026-06-29] — TIER 2 — R&D / Dignity-Required System (Sectors of Stagnation: hub card, postal sector, payments proof point)
**Change:** Three follow-ups after a thesis-coherence audit. (1) Discoverability: added a macro-companion CTA block on the DRS hub `rd/dignity/index.html` (id `drs-sectors`, model-cta pattern) right after the Rock Audit field-tool block, grouping the Last 50 Feet family (failure, field tool, macro version). The page was live but orphaned, reachable only by direct URL, /go/drs-sectors, and the reverse link. (2) Added Postal as an eighth flatlined sector on `rd/dignity/sectors-of-stagnation.html`: new bullet ("still a human to 170M doors") plus a prose paragraph cited to USPS Postal Facts (more than 170 million delivery points, 236,000+ routes, six-day human delivery; drones/robots stalled on airspace and sidewalk rules). Strong thesis fit (irreducible human presence plus regulation). (3) Converted Payments from a latent contradiction into the deliberate proof point: added a Section 04 paragraph noting money is pure information with no body and still froze, proving that losing the body only buys the chance to speed up while captured pipes and old rules can stall even a bodiless sector. No new uncited claims; USPS source confirmed free/paywall-free.
**Rationale:** Audit found payments was the one stuck sector that is bodiless, in tension with the "no body compounds" frame; naming it sharpens the three-forces model instead of leaving the contradiction quiet. Postal holds the line better than payments (physical last mile). Hub card closes the discoverability gap.
...
## [2026-06-29] — TIER 3 — R&D / Dignity-Required System (new page: Sectors of Stagnation)
**Change:** Built and deployed `rd/dignity/sectors-of-stagnation.html`, the macro companion to the Last 50 Feet essay. Dark-variant findings report on the same shell (7-item nav, GA4, JSON-LD Article, framework-header / framework-section / verdict-line / callout). Argument arc: zoom out from the warehouse rock to the economy; teardown of sectors that compounded (computing, storage, bandwidth, genome) against sectors that flatlined (construction, education, payments, aviation, transit, healthcare, mining); the why (three forces, plain-named: a human you can't remove, no real competition, too many rules); verdict tying back to the micro. Every industry stat fact-checked against a primary or authoritative source and cited inline; every citation confirmed paywall-free / reader-verifiable (BFI/Goolsbee-Syverson, OWID, Nielsen Norman, NHGRI, NCES x2, Nacha, Wikipedia x2, Annals abstract, S&P Global). Cross-links wired: forward+back to Last 50 Feet (added reverse link on `last-50-feet.html`), Rock Audit CTA box, Find-the-Rocks LinkedIn post; placeholder hrefs `#TODO-magic-middle` and `#TODO-nothing-bleeds` left for unshipped siblings. Minted `/go/drs-sectors` (campaign drs-sectors) via `_links.json` + `build_links.py`. Added the page to `sitemap.xml`. Voice: no em-dashes, no Oxford commas, plain-language pass for a non-technical ops/HR ICP. Red-teamed before publish (two "same years" stat-weld blockers fixed, agriculture/shipping counterexample pre-empted).
**Rationale:** Proves the Last 50 Feet thesis at the scale of whole industries and gives the future Sectors of Stagnation LinkedIn carousel a landing target. Innovation reached the screen and stalled at the floor; the stuck sectors are the ones where the human presence can't be removed.
**Operator:** Angie Bailey + Claude via Cowork
...
## [2026-06-29] — TIER 2 — Warehouse + Hero cards (org-scale fingerprint genericization)
**Change:** Genericized the prior-employer organizational-scale fingerprint across index.html and two case study pages. index.html warehouse cards: "$1B+ organization" to "enterprise-scale organization"; "Slack only reached 39% of the workforce. More than three-quarters of employees worked in the field" to "Slack reached a minority of the workforce. Most employees worked in the field"; "4000+ employees" to "thousands of employees"; "150+ locations, 40+ states" to "many locations across multiple states". enterprise-newsletter-architecture.html: all "hundreds of locations" to "many locations" (including meta description and JSON-LD), "hundreds of locations and dozens of states" to "many locations across multiple states", "More than three-quarters of the workforce did not sit at desks" to "Most of the workforce did not sit at desks". crisis-communications-architecture.html: hero "150 Locations." to "Many Locations.", "Slack (39% adoption rate)" to "Slack (limited adoption)", "More than three-quarters of the workforce operated in the field" to "Most of the workforce operated in the field". Performance metrics (9% to 95%+ open rate, severity levels, holding statements) retained as results, not identifiers. No structural, layout, CSS or GA4 changes.
**Rationale:** The numeric cluster (revenue scale, headcount, location and state counts, field-workforce percentages) was the strongest remaining prior-employer identifier and survived earlier word-level genericization. Replaced exact figures with scale language while keeping the case narratives intact.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-29] — TIER 2 — Warehouse (E-Commerce P&L + MBR identifier genericization)
**Change:** Genericized prior-employer identifiers on three live pages. `posts/ecommerce-pl/assets/pl-project-dossier.html`: "e-commerce parts business" / "parts e-commerce business" / "storefront parts sales" / "Equipment part expense" to generic e-commerce and product wording; "VP of E-Commerce" (x2) to "the e-commerce leader"; "billion-dollar enterprise" to "large enterprise"; the Related Work card "21 organizational constraints across 6 categories" to "more than 20 ... more than 5" (resolves the count drift with the already-genericized summit page). `posts/ecommerce-pl/ecommerce-pl.html`: dropped "parts" from "e-commerce parts business"; "VP of E-Commerce" to "the e-commerce leader". `posts/marketing-infrastructure-mbr/marketing-infrastructure-mbr.html`: removed "during the build" timing from the Evidence intro; "The director who championed... reshuffled twice within six months" to "The leader who championed... reshuffled within months". Intentionally left untouched: the P&L dossier Section 4 internal-control findings (#ERROR!, corporate-cost dumping, interest flag) pending the K&B and SOX joint decision, and the "four channels" / "4 Revenue channels unified" proof metric (weak identifier, load-bearing). No structural, layout, CSS or GA4 changes.
**Rationale:** Remove the equipment-parts product signal, the named exec role and the revenue-scale fingerprint, and align the duplicated pressure-point count with the already-genericized summit page.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-29] — TIER 2 — Warehouse + Receipt Drawer (retired two blank templates)
**Change:** Removed two downloadable blank template pages and every reference to them. Deleted `posts/marketing-infrastructure-mbr/assets/templates/cross-functional-mbr-template-blank.html` and `posts/ecommerce-pl/assets/templates/pl-project-dossier-blank.html` from the repo. Stripped the "View Blank Template" button from both parent artifact pages (`cross-functional-mbr-template.html` and `pl-project-dossier.html`), removed the Cross-Functional MBR Template res-card from the Receipt Drawer hub (`resources/index.html`), and dropped both URLs from `sitemap.xml` and both rows from `site-map.md`. All other blank templates and the filled artifact pages are unchanged.
**Rationale:** Retire the two reusable blank instruments per Angie. Removing the linking button and card plus the sitemap entries prevents broken links and orphaned crawl targets.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-29] — TIER 2 — Warehouse (MBR, E-Commerce P&L, GEO/AEO Transition, E-Commerce Summit)
**Change:** Identifiability and timing genericization pass across four case study pages, applied as targeted diffs against fresh GitHub pulls (local Portfolio Work copies were stale and missing two of the pages). (1) `posts/marketing-infrastructure-mbr/marketing-infrastructure-mbr.html`: reworded the outcome closer "The dashboard still runs. The governance doesn't." to "The infrastructure outlasted the governance built to sustain it." (past tense, no present-state claim). (2) `posts/ecommerce-pl/ecommerce-pl.html`: "Forensic assessment of the financial visibility gap." and "A forensic audit of the existing financial infrastructure..." both changed to "Internal operational review..."; underlying financial findings left intact. (3) `posts/geo-aeo-transition/geo-aeo-transition.html`: removed "during the build" timing from the Evidence intro ("Six of the twenty documents produced."); genericized "The director and VP were removed within months. Two reorganizations in fewer than six months..." to "Senior leadership turned over and the sponsoring roles were vacated. Repeated reorganizations..."; and the Outcome "Two leadership changes and two team reorganizations in fewer than six months" to "Repeated leadership turnover and team reorganizations." (4) `posts/e-commerce-summit/e-commerce-summit.html`: dropped exact counts "21 pressure points across 6 categories" (and the six named categories) to "more than 20 ... more than 5" across the artifact card, the situation paragraph and the outcome stat tile (21 to 20+). No structural, layout, CSS or GA4 changes. LinkedIn footer link retained per Angie (active job-search channel).
**Rationale:** Reduce prior-employer identifiability and remove during-employment timing and present-tense continuing-state language from the live case studies while preserving operational substance. The forensic-language swap removes the specific phrasing without stripping the financial-control findings.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-27] — TIER 2 — L&D (Good Pen hook-line copy edit)
**Change:** On ld/supply-closet-romance/good-pen.html, changed the section hook-line from "Pens are communal, slightly damp and always someone else's." to "Pens are communal, likely contaminated (post-COVID thinking) and always someone else's." Single line, exact wording per Angie.
**Rationale:** Angie's requested revision.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-27] — TIER 2 — Verdict + R&D (Verdict <-> DRS cross-links)
**Change:** Wired a bidirectional loop between two Verdict principles and the Dignity-Required System R&D hub, both of which name the system before it was published. Verdict 04 (Structure Is How You Protect People) and Verdict 05 (The Implementation Failed. Not the People.) now hyperlink their existing in-prose phrase "The Dignity-Required System" to /rd/dignity/ (no new copy, anchored on words already written). The DRS hub (rd/dignity/index.html) gains a closing reciprocal block after the Governors section pointing back up to both principles: "This system didn't start as research. It started as two convictions..." Skipped the optional links to the Crisis Comms and e-commerce summit case studies, and the Reality Before Theory -> summit link, per Angie. Reciprocal added on the hub only, not on individual DRS sub-pages. Pulled all three pages fresh from GitHub first; the stale local Portfolio Work copies lacked the 2026-05-19 SEO/JSON-LD work and would have regressed it.
**Rationale:** The principle essays named "The Dignity-Required System" as a rough draft that didn't yet exist as a destination; readers hit the name with nowhere to go. Linking principle -> proof serves the reader and the trust-gate-to-IP funnel; the hub -> principle reciprocal is where internal-link equity compounds.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-27] — TIER 1 — L&D (hook-line spacing fix)
**Change:** Fixed inconsistent spacing around the `.hook-line` emphasis pulls on all L&D articles. The rule was `margin: 1.75rem 0 0` (top only, zero bottom), so a pull sitting mid-section butted directly against the paragraph below it (0px gap) while a pull ending a section got the full section break. Changed to `margin: 1.75rem 0` for symmetric breathing room and added `.essay-body p.hook-line:last-child { margin-bottom: 0; }` so section-ending pulls don't stack a second margin on the section padding. Mid-section pulls now read as a clear break above and below; section-ending pulls keep the existing rhythm.
**Rationale:** Angie flagged that bold pulls had too much room in some places and no paragraph spacing in others within the same article. Root cause was the asymmetric (zero-bottom) hook-line margin. Single stylesheet rule covers every current and future L&D article.
**Operator:** Angie Bailey + Claude via Cowork
...
## [2026-06-27] — TIER 3 — L&D (A Supply Closet Romance, all 4 articles published; collection complete)
**Change:** Built and deployed the four leaf pages of the Dots, Pens & Commonplace Books collection (A Supply Closet Romance) under ld/supply-closet-romance/: dotted-notebook.html (01), good-pen.html (02), new-wite-out.html (03) and commonplace-book.html (04, capstone). All on the L&D dark-variant shell from Angie's final drafts: one ecru emphasis pull per section including each dek, a single collection rosso signature on the Commonplace capstone thesis ("It can't tell me which one I'll still believe in a year"), a four-term glossary per piece, and inline source links live and same-tab. Two voice fixes against the banned-word and repetition rules: "optimize" → "polish" in the Dotted Notebook, and "my standards hadn't changed, my speed had" woven into the Good Pen so its section pull has a home in the prose. Wired sibling cross-links where the prose reaches for one: Commonplace → Dotted Notebook and Wite-Out → Good Pen, plus a capstone closing line tying dotted notebook, good pen and correction tape together (the cross-case synthesis, landed once in the capstone per the single-synthesis rule). Flipped all four landing cards from placeholders to anchor-wrapped live links with no status label, and corrected the Good Pen card blurb (it described a "dry erase pen"; the piece is about a gel pen). Added four /go/ short links (scr-dotted-notebook, scr-good-pen, scr-wite-out, scr-commonplace) with linkedin / organic_social UTMs to _links.json and regenerated the stubs; updated docs/analytics-runbook.md live-slug snapshot in the same task. Added all four pages to sitemap.xml and updated site-map.md (collection 4 of 4 live).
**Rationale:** Second L&D collection complete. Same proven Em-Dash Unpacking shell and doctrine, so this was assembly from Angie's drafts, not reinvention. Rosso reserved for the one capstone thesis per the single-signature rule; the cross-case synthesis consolidated in the capstone instead of repeating per piece.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-25] — TIER 1 — Skills (settle design-system conceptual pair + prune old packages)
**Change:** Settled the open conceptual-pair question for aqb-case-study's design-system reference. Inspection confirmed the repo `_design-system.md` (232-line portable token spec) and the skill's `design-system.md` (809-line full case-study build manual with complete HTML/CSS) are different document types that share design tokens, not a stale copy. Decision: keep them distinct, detect-only; the repo spec (ultimately `styles.css` `:root`) is canonical for the tokens; neither doc is merged or overwritten. Recorded the decision in `_skills/skill-pairs.json` and `_skills/README.md`. Separately, pruned the superseded `aqb-ld-article-v1.1.skill` and `skill-version-manifest-v1.10.skill` from the `Updated Skills (Versioned)/` delivery folder, leaving one package per skill.
**Rationale:** Closing the last two parked items from the no-drift task. Promoting the 30KB build manual into the repo would duplicate `styles.css` and create a new drift surface, so detect-only against the token spec is the correct relationship.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-25] — TIER 1 — Skills (aqb-ld-article v1.2 wording fix + repo cleanup)
**Change:** Repackaged aqb-ld-article at v1.2 to carry the source-of-truth wording fix into the installed skill (its SKILL.md had still called the skill copies canonical and said the skill wins on drift; now repo-upstream / skill-derived, pointing at the CI drift check). Bumped the version-controlled source at `_skills/aqb-ld-article/SKILL.md` to 1.2 / 2026-06-25. Updated the skill-version-manifest to 1.11 (version table + changelog) and repackaged both `aqb-ld-article-v1.2.skill` and `skill-version-manifest-v1.11.skill` to `Updated Skills (Versioned)/`. Deleted two stale `.skill` binaries from the repo root (`aqb-case-study.skill`, `aqb-case-study-v2.skill`, April 25 build artifacts that were web-servable and unreferenced). No bundled template or conventions changed; the drift check still passes.
**Rationale:** Closing the loop on the no-drift task: the wording fix only mattered once it reached the running skill, and the skill-management protocol requires the version bump, manifest update and repackage of both the skill and the manifest. The root `.skill` files were dead weight reachable over HTTP.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-24] — TIER 1 — Skill sync infrastructure (no-drift check + CI)
**Change:** Made the skill-bundled copies of repo files reproducibly checkable against their repo sources, so they cannot silently drift. Added `_skills/` (Jekyll-excluded, not web-served) holding version-controlled source-of-record copies for the two skills that bundle repo content: `aqb-ld-article` (SKILL.md, plus exact copies of `_templates/ld-article-template.html` and `_templates/LD-ARTICLE-CONVENTIONS.md`) and `aqb-case-study` (SKILL.md, plus the conceptual `design-system.md` tracked against `_design-system.md`). Added `_skills/skill-pairs.json` (the pair manifest), `scripts/check_skill_drift.py` (byte-compares exact pairs, hash-flags changed conceptual sources, with an `--update-hashes` ack path), and `.github/workflows/skill-drift.yml` (runs the check on push; read-only permissions; report-only, does not gate the Pages deploy). Fixed the `aqb-ld-article` SKILL.md wording that called the skill the source of truth and the repo a copy, flipping it to repo-upstream / skill-derived. Updated `README.md` and `_build-notes.md` to document the pipeline and its honest limit. Built and verified Tier A only (detection + CI); the optional generator (`build_skills.py`) is a documented follow-up, not built.
**Rationale:** Bundled skill copies were kept in sync by a human remembering to repackage, which already bit us once (the `aqb-ld-article` template lagged the repo). This enforces the repo-as-backbone relationship mechanically: edit the canonical, the check turns red until the `_skills/` copy matches, naming the exact file to re-sync. Limit stated plainly: CI verifies the repo halves (canonical vs committed `_skills/` copy); building the `.skill` and clicking "Save Skill" stays a deliberate human step. No page, stylesheet or served file changed; the new workflow is independent of and cannot block Pages.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-24] — TIER 1 — Docs rationalization (repo-as-durable-backbone)
**Change:** Established the git repo as the durable, tool-agnostic source of truth, with the Claude skills as a derived layer re-synced from it. Rewrote the `_design-system.md` header to kill its "overrides the skills" claim and set the authority order (styles.css = runtime truth, this doc = portable spec, skills = derived copies; repo wins on conflict). Added the same role framing to `_build-notes.md` and refreshed its stale "Deployment via GitHub API" section (removed the old inline Node sample with a PAT placeholder; points to deploy.js as the canonical tool). Rewrote `README.md` as the documentation index: a "where each thing lives" table for the durable repo layer plus the derived skill layer. KEPT both `_design-system.md` and `_build-notes.md` as durable records rather than collapsing them to pointers (the originally-proposed pointer for build-notes was revised: pointer-izing would push build knowledge into the skill+memory layer only, the tool-coupling risk this whole pass guards against).
**Rationale:** Angie's resilience concern: foundational, tool-agnostic information (design tokens, build conventions) must not live only inside a Claude-specific skill format. Repo is portable, git-versioned, and survives tool changes. SSOT with one canonical home per concern, derived copies downstream, README as entry point. No skill or manifest changes.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-24] — TIER 1 — Repo cleanup (--message deleted, registry retired)
**Change:** Deleted the stray root file `--message` (a 31KB accidental duplicate of the Growth Collapse Math DRS page that was serving live at angieqbailey.com/--message with no sitemap entry). Collapsed `_site-registry.md` from a full second file inventory into a short pointer to `site-map.md`, the designated source of truth, to end the two-competing-inventories problem (the registry had drifted, missing Stars & Bars and the Rock Audit).
**Rationale:** Repo audit. `--message` was junk from a botched command and a minor duplicate-content snag. Two inventories both claiming authority is how a repo gets confusing; one canonical inventory (`site-map.md`) now stands.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-24] — TIER 1 — Global CSS (consolidate page-scoped styles + unify links)
**Change:** Moved the repeated per-page `<style>` blocks into global `styles.css`. The L&D emphasis pull (`.essay-body p.hook-line` + `.is-rosso`, with the specificity that beats `.essay-body p`), the inline-link color, and `.scale-note` now live once in the shared stylesheet. Removed the duplicated blocks from all four Em-Dash Unpacking articles and the L&D template; updated the conventions doc. Unified inline content-link color site-wide to quetzal-bright with a 1px underline (was default browser purple on DRS `.body-text` and Verdict `.essay-body` pages, teal only on L&D): selector covers `.essay-body`, `.body-text`, `.glossary-body`, `.scale-note`, `.doc-lede`, `.framework-lede`, `.working-def`, `.section-body`, `.ld-lede`. Corrected the stale site-map note: `styles.css` is formatted, not minified.
**Rationale:** One source of truth instead of five copies, and consistent links across the whole site. The DRS and Verdict inline citations were rendering purple, clashing with the dark Brutalist palette. The four articles are visually unchanged (the global rules match what their page blocks did); the visible change is teal links on the DRS findings pages and any Verdict essay with inline links.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-24] — TIER 1 — Infrastructure (L&D article template + conventions)
**Change:** Added two reference files under `_templates/`: `ld-article-template.html` (the proven dark-variant doc/essay shell with placeholders and inline build rules) and `LD-ARTICLE-CONVENTIONS.md` (the written doctrine converged across the four Em-Dash Unpacking articles, plus a paste-ready block for the project instructions). Noted both in site-map.md. These let the next L&D collection (Supply Closet Romance) start from a known-good file instead of reverse-engineering the shell from Verdict and DRS pages.
**Rationale:** Lessons-forward cleanup. The shell and doctrine were rebuilt four times this session and lived only in the deployed files. Codified once. Does not change any live page. Still open (decisions, not done here): moving the page-scoped CSS into global styles.css, and standing up an aqb-ld-article skill.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-24] — TIER 3 — L&D (Em-Dash Unpacking, Article 04 published; collection complete)
**Change:** Built and deployed ld/em-dash-unpacking/stars-and-bars.html (Article 04, "Stars & Bars: Straight Out of 1861 Manassas") on the shell: five doc-sections (Actual Flags, Resurrection, Escalation, Pattern, Comparison), six ecru emphasis pulls (one per section plus the dek, no rosso given subject), a manufactured-nostalgia timeline rendered as bold-led entries, the scale-caveat note, and a six-term glossary. The Comparison section is the collection capstone and now carries the cross-case synthesis (swastika/flag/em-dash plus the surviving ellipsis), so the four-way comparison lands once here instead of repeating in every article. Wired live links to Articles 01, 02 and 03. Wired the previously plain-text Confederate-flag references in Articles 01 (Charge), 02 (Survival) and 03 (Pattern) to point here, so all four pieces now cross-link cleanly in every direction. Wired the Stars & Bars card live on the landing page (last Em-Dash Unpacking placeholder retired; collection 4 of 4 live). Added Article 04 to sitemap.xml and updated site-map.md (collection complete). Single "Back to L&D" bottom nav; inline "one of four pieces" collection link kept in the scale note.
**Rationale:** Final article in the collection. Capstone synthesis consolidated here per the dedup decision made on Article 03. Confederate-flag cross-links completed now that the destination exists.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-24] — TIER 2 — L&D (Article 03 feedback edits, deacademized)
**Change:** Four edits to Article 03, reworded plainer per Angie. (1) Dedupe: cut the Pattern section's repeated thesis and the Nazis/Dixiecrats/OpenAI triad (Article 01's move, reserved now for an Article 04 capstone) and replaced it with one paragraph on what's distinct about the swastika, its speed and totality, plus the one-directional asymmetry. (2) Added a living-stakeholders beat before the close: "The Ancient Greeks are an easy name for the loss, because they're safely gone. The people who actually live with it are still here: every Hindu, Buddhist and Jain..." so the title's pun no longer subordinates the real ongoing cost. (3) Softened the independent-invention overclaim from "Nobody passed it around" to "As far as anyone can tell, most of them landed on the same rotating cross on their own." (4) Reworded the Svastika glossary gloss to drop the parenthetical-quote construction (resolving the comma question) and read plainer: "Su means good, asti means being, so the whole thing means well-being or good fortune."
**Rationale:** Angie's feedback pass. Less academic phrasing, intent preserved. The triad belongs in one place, not four.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-24] — TIER 3 — L&D (Em-Dash Unpacking, Article 03 published)
**Change:** Built and deployed ld/em-dash-unpacking/hitler-vs-ancient-greeks.html (Article 03, "The Case of Hitler vs. The Ancient Greeks") on the shell: four doc-sections (Original, Theft, Asymmetry, Pattern), five ecru emphasis pulls (one per section plus the dek), a six-term glossary, and a scale-caveat note set apart with a rosso left border ("nothing here treats genocide as the equal of a writing habit"). No rosso on the pulls, deliberately, given the subject. In-body em-dash reference links live to Article 01; Confederate flag stays plain text until Article 04. Wired the Hitler vs. Greeks card live on the landing page and wired the previously plain-text "swastika" references in Article 01 (Charge) and Article 02 (Survival) to point here. Added Article 03 to sitemap.xml and updated site-map.md (3 of 4 live). Bottom nav is the single "Back to L&D" per the dropped-collection-link convention; kept the inline "one of four pieces" collection link in the scale note.
**Rationale:** Article 03 of 4 is fact-checked and build-ready. The scale caveat is in the body, so the light-to-heavy jump across the collection reads as deliberate argument. Emphasis pulls kept ecru, not rosso, so red is not used to dress up Holocaust content.
**Operator:** Angie Bailey + Claude via Cowork
...
## [2026-06-24] — TIER 3 — L&D (Em-Dash Unpacking, Article 02 published)
**Change:** Built and deployed ld/em-dash-unpacking/semiotic-thief-charges-dont-stick.html (Article 02, "Semiotic Thief Charges Don't Stick") on the Article 01 shell: four doc-sections (Origin, Migration, Survival, Irony), two ecru hook-line pulls ("The answer isn't luck. It's structural. Four reasons." and the closing "Semiotic thief charges don't stick to three dots."), four-term glossary, demonstrative ellipses preserved as &hellip; per the integration notes, single L&D back-link only. In-body em-dash references link live to Article 01 (two spots); swastika and Confederate flag stay plain text until Articles 03-04 ship. Wired the Semiotic Thief card live on ld/infrastructure-of-a-life.html. Cross-linked Article 01's "Article 2 makes the case for why" live and dropped Article 01's "Back to Em-Dash Unpacking" collection link (kept "Back to L&D"). Added Article 02 to sitemap.xml and updated site-map.md (2 of 4 live).
**Rationale:** Article 02 of 4 is fact-checked and build-ready. Reused Article 01's emphasis system. Removed one Oxford comma in the ambiguity-reason list ("coyness, trailing off or an invitation") to meet the no-Oxford-comma rule. No rosso on Article 02 yet; the closing title-callback is the natural candidate if Angie wants a signature accent.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-24] — TIER 2 — L&D (Em-Dash Unpacking, Article 01 close + Verdict rewrite)
**Change:** Two edits in the Verdict section. (1) The closing emphasis line was too long and crowded at hook-line size, so chopped it: the highlight is now just "Five hundred years of utility." and the rest ("Assassinated by a language model that didn't know what it was doing and a discourse machine that didn't care about the collateral damage.") dropped to normal body weight beneath it. (2) Rewrote the opening Verdict paragraph from the "I stopped using em-dashes... starts filling space" version to the broadcast-habit origin: "I never really used the em-dash. Not a principled stand, just muscle memory from years of writing TV news copy... It wasn't. It was broadcast." Sets up the next paragraph's three-dot ellipsis payoff.
**Rationale:** Per Angie. Highlighting one short sentence reads cleaner than a five-line block. The rewrite grounds the no-em-dash habit in her TV news background rather than a noticed-it-in-drafts story, which lands truer and feeds the ellipsis line.
**Operator:** Angie Bailey + Claude via Cowork

## [2026-06-24] — TIER 2 — L&D (Em-Dash Unpacking, Article 01 fact + unit fixes)
**Change:** Two content fixes. (1) Unit drift: changed the hook from "six times per paragraph" to "six times a page" so the hook plants the floor and the body line "Six, eight, ten per page." escalates off the same unit. Synced the meta description and og:description to "a page" to match. Body escalation line left untouched. (2) Single-source keystone: expanded the Sam Altman paragraph in The Hit to add an earlier corroborating admission ("copped to it, more than once... He'd already told a podcast host his team leaned into em-dashes because users seemed to like them, then admitted there were now too many") carrying a link to the 99% Invisible em-dash episode. Sukhareva's named 10x measurement stays intact, so it now reads as one data point beside Altman's own admission rather than the whole wall.
**Rationale:** Both were flagged in the Article 01 feedback pass. The unit match makes the climb deliberate. The second source keeps the keystone density claim from resting on a single Substack count.
**Operator:** Angie Bailey + Claude via Cowork
