## [2026-05-03] — TIER 2 — R&D (Section 03) — SOP Playbook Artifacts
**Change:** Converted SOP Playbook Builder's three Google Doc links to self-hosted artifact pages. Created plain-language-guide.html (dark-variant artifact page with full four-pass guide content), pro-guide.html (dark-variant artifact page with technical vocabulary version), and capture-template-blank.html (ecru blank template with print button). Updated SOP landing page to link to self-hosted pages instead of Google Docs. Removed target="_blank" from all resource links. Site-map updated with 3 new pages.
**Rationale:** Self-hosted pages maintain brand consistency, prevent external dependency on Google Docs, enable print/save-as-PDF workflow matching Warehouse artifact pattern, and keep visitors on-site.
